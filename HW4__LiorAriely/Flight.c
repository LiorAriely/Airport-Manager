...
int saveFlightToCompressedFile(const Flight* pF, FILE* fp)
{
	CHECK_RETURN_0(pF);

	BYTE data[6] = { 0 };
	int lenDst = strlen(pF->nameDest);
	int lenSrc = strlen(pF->nameSource);
	int typePlane = (int)pF->thePlane.type;
	data[0] = lenSrc << 3;
	data[0] |= lenDst >> 2 ;

	data[1] = (lenDst & 0x3) << 6;
	data[1] |= (typePlane << 4);
	data[1] |= (pF->date.month);

	data[2] = ((pF->thePlane.code[0] - 0x41) << 3)  ;
	data[2] |= ((pF->thePlane.code[1] - 0x41 )>>2) & 0x7  ;

	data[3] = (((pF->thePlane.code[1] - 0x41 )& 0x3) << 6)  ;
	data[3] |= ((pF->thePlane.code[2] - 0x41 )<< 1)  ;
	data[3] |= ((pF->thePlane.code[3] - 0x41) >> 4) & 0x1  ;

	data[4] = ((pF->thePlane.code[3] - 0x41) & 0xF) << 4;
	data[4] |= (pF->date.year)-2021;
	...
	if (fwrite(pF->nameDest, sizeof(char), lenDst, fp) != lenDst)
		return 0;
...
int loadFlightFromCompressedFile(Flight* pF, const AirportManager* pManager, FILE* fp)
{
	...
	int lenDst = ((data[0] & 0x7)<<2) | ((data[1] >> 6) & 0x3);


	int typePlane = (data[1] >> 4) & 0x3;
	pF->thePlane.type = (ePlaneType)typePlane;
	pF->date.month = data[1] & 0xF;

	pF->thePlane.code[0] = (char)(0x41 + ((data[2] >>3 ) & 0x1F));
	pF->thePlane.code[1] = (char)(0x41 + (((data[2] & 0x07)<<2) | ((data[3] >>6) & 0x3)));
	pF->thePlane.code[2] = (char)(0x41 + ((data[3] & 0x3E) >> 1));
	pF->thePlane.code[3] = (char)(0x41 + (((data[3] & 0x1)<<4) | ((data[4]  >>4 ) & 0xF)));
	
	pF->date.year = 0x7E5 + (data[4] & 0x0F);
	...
	pF->nameDest = (char*)calloc(lenDst +1, sizeof(char));
	if (!pF->nameDest)
		return 0;

	if (fread(pF->nameDest, sizeof(char), lenDst, fp) != lenDst)
	{
		free(pF->nameDest);
		return 0;
	}
