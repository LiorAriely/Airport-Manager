...
int		initAirlineFromFile(Airline* pComp, AirportManager* pManager, const char* fileName, int isCompressed)
{
	...
	{
		if (loadAirlineFromCompressedFile(pComp, pManager, fileName))
			return initDateList(pComp);
	}
	return 0;
}
...
int saveAirlineToCompressedFile(const Airline* pComp, const char* fileName)
{
	...
	BYTE data[2] = { 0 };
	int sortNum = (int)sortOptStr;
	int length = (int)strlen(pComp->name);
...
int loadAirlineFromCompressedFile(Airline* pComp, const AirportManager* pManager, const char* fileName)
{
	FILE* fp;
	...
	int sortNum = 0;
	int length = 0;
	pComp->flightCount = ((data[0] << 1) & 0xFF) | ((data[1] >> 7 )& 0x1);
	sortNum = ((data[1] >> 4) & 0x7);
	pComp->sortOpt = (eSortOption)sortNum;
	length = (data[1] & 0xF);
